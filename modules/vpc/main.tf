...
resource "aws_nat_gateway" "main" {
  ...
  tags = merge(
    var.tags,
    {
      Name = var.single_nat_gateway ? "${var.vpc_name}-nat" : "${var.vpc_name}-nat-${var.azs[count.index]}"
    }
  )
...
resource "aws_route_table" "private" {
  ...
  tags = merge(
    var.tags,
    {
      Name = var.single_nat_gateway || !var.enable_nat_gateway ? "${var.vpc_name}-private-rt" : "${var.vpc_name}-private-rt-${var.azs[count.index]}"
    }
  )
}
